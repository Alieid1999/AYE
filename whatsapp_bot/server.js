    ...
    try {
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (isConflict) {
                    console.log(`⚠️ WhatsApp connection replaced/conflict (Status 440). Waiting 45 seconds before reconnecting to ensure single instance...`);
                    setTimeout(connectToWhatsApp, 45000);
                } else if (shouldReconnect) {
                    console.log(`⚠️ WhatsApp connection closed (Status: ${statusCode}). Reconnecting in 15s...`);
        ...
        sock.ev.on('messages.upsert', async (m) => {
            ...
            if (matchedOrderId) {
                ...
                if (updateSuccess) {
                    ...
                    // Forward the exact full WhatsApp message text directly to Telegram with buttons
                    try {
                        const fullMsgText = String(text || '').trim();
                        const payload = {
                            chat_id: TELEGRAM_ADMIN_CHAT_ID,
                            text: fullMsgText || `📦 Order Details #${matchedOrderId}`,
                            parse_mode: 'Markdown',
                            reply_markup: buildTelegramOrderKeyboard(matchedOrderId),
                            disable_web_page_preview: true
                        };
                        await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, payload, { timeout: 10000 });
                        console.log(`🟢 Forwarded exact WhatsApp order message to Telegram for Order #${matchedOrderId}`);
                    } catch (tgErr) {
                        console.error('⚠️ Failed to forward WhatsApp order message to Telegram:', tgErr.message);
                    }
                } else {
...
async function renderTelegramOrderDetailsMessage(chatId, messageId, orderId) {
    ...
    let itemsText = '';
    for (const item of items) {
        itemsText += `- ${item.title || 'Item'} (x${item.quantity || 1})\n`;
    }

    const custPhone = cust.phone ? `+${cust.phone}` : 'Auto-linking via WhatsApp...';
    const detailsMsg = 
    ...
    try {
        ...
        console.error('Failed to render Telegram order details message with Markdown, retrying plain text:', err.message);
        try {
            if (messageId) {
                await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/editMessageText`, {
                    chat_id: chatId,
                    message_id: messageId,
                    text: detailsMsg.replace(/\*/g, ''),
                    reply_markup: keyboard
                }, { timeout: 10000 });
            } else {
                await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
            ...
            console.error('Failed plain text retry:', retryErr.message);
        }
    }
}
...
async function pollTelegramUpdates() {
    ...
    try {
        ...
        if (err.response && err.response.status === 409) {
            console.log('⚠️ Telegram 409 Conflict: Another bot instance is polling this token. Dropping webhooks and backing off for 10 seconds...');
            try {
                await axios.post(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/deleteWebhook?drop_pending_updates=true`, {}, { timeout: 10000 });
            } catch (e) {}
            nextDelay = 10000;
